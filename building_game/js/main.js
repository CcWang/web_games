...
function isAdjacent(v1,v2){
	...
	if (Math.abs(v1.position.y - v2.position.x) === 50) {
		adjacentPos ++;
	}
...
function loadProgress(){
	...
	for(var i in loadedObj){
		...
		document.getElementById('height').innerHTML = calcHeight();
		document.getElementById('areas').innerHTML = build_areas(voxel);
		document.getElementById('numOfConnect').innerHTML = build_connect(voxel);
			
	}
	document.getElementById('height').innerHTML = calcHeight();
	document.getElementById('numOfCubes').innerHTML = objects.length-1;
	...
	if ( intersects.length > 0 ) {
		...
		if ( isShiftDown ) {
			if ( intersect.object != plane ) {
				...
				document.getElementById('numOfConnect').innerHTML = remove_connect();
				document.getElementById('height').innerHTML = calcHeight();
			}
		// create cube
		} else {
